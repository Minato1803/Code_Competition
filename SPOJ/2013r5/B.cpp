...
//declare
ll tl[105], dis[105],r[105],g[105];
ll n,l;
//============//

void update(ll time, ll pos)
{
	for(ll i=pos;i<n;i++)
	{
		if(tl[i]<time)
		{
			...
			if(time%(r[i]+g[i])==0)
			{
				tl[i] = (r[i]+g[i])*(time/(r[i]+g[i])) - time;
			}
			else
			{
				tl[i] = (r[i]+g[i])*(time/(r[i]+g[i])+1) - time;
			}
		}
		else
		{
...
{	
	...
	for(ll i=1;i<n;i++)
	{
		if(tl[i-1]>g[i-1])
		{
			...
			ans+= (tl[i-1]-g[i-1]);
			update(tl[i-1]-g[i-1], i-1);
		}
		ll tmp = dis[i] - dis[i-1];
		ans+=tmp;
		update(tmp,i);
	}
	if(tl[n-1]>g[n-1])
