...
string a,b;
char c;
vp(string, string) sorts;
string name;
//-----------------------------
bool cmpName(pair<string, string> a, pair<string, string> b)
{
	if(a.snd == b.snd)
		return a.fst<b.fst;
	else	
...
void Solves()
{
	testCase
	{
		...
		sort(all(sorts),cmpName);
		flr(i,0,sorts.size())
		{
			cout<<sorts[i].fst<<" "<<sorts[i].snd<<endl;
		}
		flr(i,0,sorts.size())
		{
			if(s.find(sorts[i].fst)!=-1)
			name+=" ";
			name+=
		}
