...
void Solves()
{
	...
	if(posEnd == -1 || a.length()<(posEnd+1))
	{
		cout<<a<<b<<endl;
		return;
	}
//	cout<<a.length()<<" "<<b.length()<<" "<<posEnd<<endl;
	ll posBegin = a.length() - (posEnd+1);
//	cout<<a.length()<<" "<<posBegin<<" "<<a[a.length()+1 - posEnd]<<endl;
	if(a[posBegin] == b[0])
	{
		cout<<a<<b.substr(posEnd+1);
	}
	else
	{
		cout<<a<<b<<endl;
